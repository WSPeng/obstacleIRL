% Interpolate states (or action) in a discretized MDP.
...
next_state = bsxfun(@minus,next_state,bounds(1,:)); % next_state Subtract lower bounds.
lidx = min(cells+1,max(1,floor(bsxfun(@rdivide,next_state,step_size))+1)); % lower index
uidx = min(cells+1,max(1,ceil(bsxfun(@rdivide,next_state,step_size))+1)); % upper index
